...
pharm = pd.read_csv('source_data/dataset_2dpharm.csv', header = None)
pharm.columns = ['SMILES'] + [f'2d_Pharm_{i}' for i in range(1, pharm.shape[1])]

# Merge the datasets based on the SMILES column
merged_data_with_fingerprints = pd.merge(data_raw, pharm, on='SMILES', how='inner')
...
merged_output_file = 'tanimoto/hierarch_cluster/2d_pharm/merged_clusters_complete_2dpharm.csv'
